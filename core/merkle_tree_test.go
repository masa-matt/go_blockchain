...
func TestNewMerkleTreeOddLevels(t *testing.T) {
	data := [][]byte{
		[]byte("node1"),
		[]byte("node2"),
		[]byte("node3"),
	...
	assert.Equal(t, rootHash, fmt.Sprintf("%x", mTree.RootNode.Data), "Merkle tree root hash is not correct")
}

func TestNewMerkleTreeEvenNodesPairs(t *testing.T) {
	data := [][]byte{
		[]byte("node1"),
		[]byte("node2"),
		[]byte("node3"),
	...
	// Level 4
	n13 := NewMerkleNode(n11, n12, nil)

	rootHash := fmt.Sprintf("%x", n13.Data)
	mTree := NewMerkleTree(data)

	assert.Equal(t, rootHash, fmt.Sprintf("%x", mTree.RootNode.Data), "Merkle tree root hash is not correct")
}

func TestNewMerkleTreeEvenLevels(t *testing.T) {
	data := [][]byte{
		[]byte("node1"),
		[]byte("node2"),
		[]byte("node3"),
